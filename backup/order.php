...
$sql = "SELECT master_kecamatan.id, master_kecamatan.nama as nama, master_kota.nama as kota FROM master_kecamatan LEFT JOIN master_kota ON master_kecamatan.kota_id = master_kota.id ORDER BY master_kecamatan.nama ASC";
...
<body>
		...
		<div class="col-sm-12">
			<div class="orders mb-3">
				<div class="title-orders">
					Reservasi Online <b>Door to Door</b>
				</div>
	...
	<form action="order-process.php" method="post" enctype="multipart/form-data">
		<div class="row" style="width:1176px; margin:0px auto; margin-bottom:100px;">
			<div class="col-sm-7">
				<div class="order2">
					<input type="hidden" name="tipe" value="Door to Door">
					...
					<table width="100%">
						<tr>
							...
							<td width="40px" style="padding: 10px;">
								<?php if ($_GET['from'] == "kecamatan" || $_GET['from'] == "") { ?>
									<a href="/order.php?from=bandara">
									<?php } else { ?>
										<a href="/order.php?from=kecamatan">
										<?php } ?>
					...
					<div class="box-order fs-14 mt-3">
						<table width="100%">
							<tr>
								...
								<td class="fs-14" width="60%" id="textUmum">0 Umum</td>
								<td class="fs-14 mb-3" style="text-align:right" id="totalUmum">Rp. 0</td>
							</tr>
							<tr>
								<td width="30px"><img src="assets/images/icons/people-2.png"></td>
								<td class="fs-14" width="60%" id="textMahasiswa">0 Mahasiswa</td>
								<td class="fs-14" style="text-align:right" id="totalMahasiswa">Rp. 0</td>
							</tr>
							<tr>
								<td colspan="3">
									<hr>
							...
							<tr>
								...
								<td class="fs-14" style="text-align:right; color:#EA5455;" id="totalDiskon">Rp. 0</td>
							</tr>
							<tr>
								<td width="30px"></td>
								<td class="fs-14" style="text-align:right" width="50%">Total</td>
								<td class="fs-14" style="text-align:right" id="total">Rp. 0</td>
							</tr>
						</table>
						<hr>
						<div class="row">
			...
			<div class="col-sm-7 mt-4">
				<div class="order2">
					<table width="100%">
						<tr>
							<td colspan="2">
								<div style="font-size:22px;" class="mb-4 mt-2">Detail Penumpang</div>
							</td>
							<td colspan="2">
								<div class="mb-4 mt-2">
									<select class="form-control mt-2 fs-14 mb-3 p-10" id="penumpangCount" onchange="penumpang()">
						...
						<tbody id="penumpang">
							<tr>
								<td colspan="4">
									<div class="row">
										<div class="col-sm-6">
											<div class="fs-14">Penumpang 1</div>
											<div><input type="text" name="nama_penumpang[]" class="form-control mt-2 fs-14 mb-3 p-10" placeholder="Nama Lengkap" id="penumpang-1" onkeyup="calculate()"></div>
										</div>
										<div class="col-sm-6">
											<div class="fs-14">Jenis</div>
											<select class="form-control mt-2 fs-14 mb-3 p-10" onchange="checkJenis(this,1)" id="jenis-1" name="jenis_penumpang[]">
												<option value="Umum">Umum</option>
												<option value="Mahasiswa">Mahasiswa</option>
											</select>
										</div>
										<div class="col-sm-12" id="upload-1" style="display:none">
											<div class="fs-14">Upload Kartu Mahasiswa</div>
											<div><input type="file" class="form-control mt-2 fs-14 mb-3 p-10" placeholder="Nama Lengkap" id="file-1"></div>
										</div>
									</div>
								</td>
							</tr>
	...
	<script>
		...
		function calculate() {
			...
			for (var i = 1; i <= parseInt($("#penumpangCount").val()); i++) {
				var penumpang = $("#penumpang-" + i).val();
				var jenis = $("#jenis-" + i).val();
				if (jenis == "Umum" && penumpang != "") {
					umum++;
					totalUmum += 250000;
				}
				if (jenis == "Mahasiswa" && penumpang != "") {
					mahasiswa++;
					totalMahasiswa += 250000;
					totalDiskon += 50000;
				}
			}
			$("#textUmum").html(umum + " Umum");
			$("#totalUmum").html("Rp. " + nominal(totalUmum));
			$("#textMahasiswa").html(mahasiswa + " Mahasiswa");
			$("#totalMahasiswa").html("Rp. " + nominal(totalMahasiswa));
			$("#totalDiskon").html("Rp. " + 0);
			if (totalDiskon > 0) {
				$("#totalDiskon").html("-Rp. " + nominal(totalDiskon));
			}
			$("#total").html("Rp. " + nominal(totalUmum + totalMahasiswa - totalDiskon));
			...
			for (var i = 1; i <= parseInt($("#penumpangCount").val()); i++) {
				html += `<tr>
					<td colspan="4">
					    <div class="row">
					        <div class="col-sm-6">
        						<div class="fs-14">Penumpang ` + i + `</div>
        						<div><input type="text" name="nama_penumpang[]" class="form-control mt-2 fs-14 mb-3 p-10" placeholder="Nama Lengkap" id="penumpang-` + i + `" onkeyup="calculate()"></div>
    						</div>
					        <div class="col-sm-6">
        						<div class="fs-14">Jenis</div>
    						    <select class="form-control mt-2 fs-14 mb-3 p-10" onchange="checkJenis(this,` + i + `)" id="jenis-` + i + `" name="jenis_penumpang[]">
    						        <option value="Umum">Umum</option>
    						        <option value="Mahasiswa">Mahasiswa</option>
    						    </select>
    						</div>
					        <div class="col-sm-12" id="upload-1" style="display:none">
        						<div class="fs-14">Upload Kartu Mahasiswa</div>
        						<div><input type="file" class="form-control mt-2 fs-14 mb-3 p-10" placeholder="Nama Lengkap" id="file-` + i + `"></div>
    						</div>
						</div>
					</td>
				</tr>`;
		...
		$(document).ready(function() {
			...
			if (urlParams.get('from') === 'kecamatan') {
				if (urlParams.get('district')) {
					$("#from").val(urlParams.get('district'));
				}
				if (urlParams.get('airport')) {
					$("#to").val(urlParams.get('airport'));
				}
			} else if (urlParams.get('from') === 'bandara') {
				if (urlParams.get('district')) {
					$("#to").val(urlParams.get('district'));
				}
				if (urlParams.get('airport')) {
					$("#from").val(urlParams.get('airport'));
				}
			}

			// set penumpang
			if (urlParams.get('penumpang')) {
				$("#penumpangCount").val(urlParams.get('penumpang'));
				calculate();
				penumpang();
			}

			if (urlParams.get('date')) {
				$("#akhir_tanggal").val(urlParams.get('date'));
				$("#awal_tanggal").val(urlParams.get('date'));
				waktuJemput();
			}

			if (urlParams.get('time')) {
				$("#akhir_waktu").val(urlParams.get('time'));
				waktuJemput();
			}
		});
	</script>
